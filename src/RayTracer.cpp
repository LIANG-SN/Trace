	...
	double range_x, double range_y, int& num_samples)
{
	...
	vec3f LT = trace(scene, center_x - range_x / 2, center_y + range_y / 2);
	vec3f LB = trace(scene, center_x - range_x / 2, center_y - range_y / 2);
	vec3f RT = trace(scene, center_x + range_x / 2, center_y + range_y / 2);
	vec3f RB = trace(scene, center_x + range_x / 2, center_y - range_y / 2);
...
void RayTracer::tracePixel( int i, int j )
{
	...
	else if (traceUI->super_sampling) // if not adaptive but average down
	{
		...
		col = trace(scene, double(i) / double(buffer_width), double(j) / double(buffer_height));
